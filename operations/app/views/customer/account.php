...
<style>
/* Custom scrollbar styling for account sidebar */
.col-md-5.col-lg-4::-webkit-scrollbar {
    width: 8px;
}

.col-md-5.col-lg-4::-webkit-scrollbar-track {
    background: #f1f1f1;
    border-radius: 10px;
}

.col-md-5.col-lg-4::-webkit-scrollbar-thumb {
    background: #198754;
    border-radius: 10px;
}

.col-md-5.col-lg-4::-webkit-scrollbar-thumb:hover {
    background: #146c43;
}
...
#main-account-content::-webkit-scrollbar-track {
    background: #f1f1f1;
    border-radius: 10px;
}

#main-account-content::-webkit-scrollbar-thumb {
    background: #888;
    border-radius: 10px;
}

#main-account-content::-webkit-scrollbar-thumb:hover {
    background: #555;
}
</style>

<div class="container-fluid ">  <?php if (!empty($data['account_number_error'])): ?>
  ...
  <?php endif; ?>
    <div class="row" style="height: calc(100vh - 100px);">
        <!----------------------- LEFT SIDEBAR ------------------------------------------------------------------------------------>
        <div class="col-md-5 col-lg-4 pt-5 px-5" style="background-color: #D9D9D94D; height: 100%; overflow-y: auto;">
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('#showHideAccountModal .form-check-input').forEach(toggle => {
        toggle.addEventListener('change', function() {
            const accountId = this.id.replace('showHideToggle_', '');
            const isHidden = !this.checked; // If unchecked, account is hidden

            // Send AJAX request to save the state
            fetch('<?= URLROOT; ?>/customer/toggleAccountVisibility', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    console.error('Failed to update account visibility:', data.message);
